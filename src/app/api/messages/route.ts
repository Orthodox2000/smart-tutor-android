...
export async function POST(request: Request) {
  try {
    ...
    if (role === 'student') {
      const allowed = audience.every((a: string) => a === 'educator' || a === 'admin');
      if (!allowed) {
        return NextResponse.json({ error: 'Students can only message educators or admins.' }, { status: 403 });
      }
      if (userIds.length === 0) {
        return NextResponse.json({ error: 'Please select at least one recipient.' }, { status: 400 });
      }
    } else if (role === 'educator') {
      const allowed = audience.every((a: string) => a === 'admin' || a === 'student');
      if (!allowed) {
        return NextResponse.json({ error: 'Educators can only message admins or students.' }, { status: 403 });
      }
    }
    ...
    logAction({
      action: 'create',
      category: 'messages',
      details: `Message sent (${title})`,
      metadata: { messageId: message._id?.toString?.(), title, channel, audience, userIds },
      request,
      userId: session.id,
